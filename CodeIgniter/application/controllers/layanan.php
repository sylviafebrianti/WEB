...
class Layanan extends CI_Controller {
	public function index(){
		...
		$data= $this->mod->GetTable('layanan_cuci');
		$this->load->view('view_layanan', array('data'=>$data));
	}
	public function insert(){
		if(isset($_POST['submit'])){
			$mod = $this->mod->kode_otomatis('layanan_cuci');
			$ID_LAYANAN =$mod;
			...
			$this->mod->InsertData('layanan_cuci', $data);
			redirect('layanan');
		}
		else {
	...
	public function update(){
		if(isset($_POST['submit'])){
			...
			$HARGA_LAYANAN_CUCI = $this->input->post('HARGA_LAYANAN_CUCI');
			$data = array('ID_LAYANAN'=>$ID_LAYANAN, 'JENIS_LAYANAN_CUCI'=>$JENIS_LAYANAN_CUCI, 'HARGA_LAYANAN_CUCI'=>$HARGA_LAYANAN_CUCI);
			$this->mod->UpdateData('layanan_cuci', $data, $id);
			redirect('layanan');
		}
		else {
			$id = $this->uri->segment(3);
			$data = $this->mod->getByID('layanan_cuci','ID_LAYANAN', $id)->row_array();
	...
	public function delete(){
		$id = $this->uri->segment(3);
		$this->mod->DeleteData('layanan_cuci','ID_LAYANAN',$id);
		redirect('layanan');
	}
}
?>